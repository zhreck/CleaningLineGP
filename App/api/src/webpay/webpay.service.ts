...
export class WebpayService {
    ...
    constructor(
        ...
        this.logger.log(`Webpay Plus configurado en modo: ${environment === Environment.Integration ? 'Integration' : 'Production'}`);
    }
    ...
    async createTransaction(
        ...
        const returnUrl = this.configService.get<string>('WEBPAY_RETURN_URL') || 'http://localhost:3000/pago/resultado';
        ...
        try {
            ...
            this.logger.log(`Transacción creada exitosamente: token=${response.token}`);
    ...
    async commitTransaction(
        ...
        try {
            ...
            const isAuthorized = response.status === 'AUTHORIZED' && response.response_code === 0;
