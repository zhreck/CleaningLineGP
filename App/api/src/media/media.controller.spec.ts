...
describe('MediaController', () => {
        ...
        it('should return a URL for any valid image file (Property 22)', async () => {
            ...
            await fc.assert(
                fc.asyncProperty(
                    ...
                    fc.record({
                        originalname: fc.string({ minLength: 1, maxLength: 50 }).map(s => `${s}.jpg`),
        ...
        describe('file validation errors', () => {
            it('should reject invalid file types', async () => {
                ...
                mockMediaService.upload.mockRejectedValue(
                    new BadRequestException('Invalid file type. Only images are allowed (JPEG, PNG, GIF, WEBP)'),
                );
                ...
                fc.asyncProperty(
                    ...
                    async (imageData) => {
                        ...
                        const mockUrls = imageData.map(
                            (img) => `http://localhost:9000/products/${img.timestamp}-${img.fileName}`,
                        );
    ...
    describe('delete', () => {
        ...
        it('should reject delete without file URL', async () => {
            ...
            await expect(controller.delete('')).rejects.toThrow('File URL is required');
        });
